#데이터 아무런 보정 없이 clustering 해보기

setwd("F:/MICSSS_Analysis/20220103 spectre/")
library(dplyr)
library(Spectre)
...
fwrite(cell.dat, "cell.data.csv") # 값 보정 없이 meta랑 합치기만 한 data
...
cellular.cols <- names(cell.dat)[c(2:4, 6, 8:13)] #모든 단백질값 
cellular.cols

## Columns for clustering
clustering.cols <- names(cell.dat)[c(3, 4, 12)]#CD3, CD8, Pax5
clustering.cols
...
### Run clustering
cell.JN <- run.flowsom(cell.dat, clustering.cols, meta.k = metak)
